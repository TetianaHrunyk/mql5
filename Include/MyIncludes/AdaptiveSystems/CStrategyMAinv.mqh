...
//+------------------------------------------------------------------+
//| Class CStrategyMAinv for implementation of virtual trading       |
//| by the strategy that is opposite to the one of moving averages   |
//+------------------------------------------------------------------+
class CStrategyMAinv : public CSampleStrategy
  {
protected:
   int               m_handle;   // handle of the Moving Average (iMA) indicator
   int               m_period;   // period of the Moving Average indicator
   double            m_values[]; // array for storing the values of indicator
public:
...
//+------------------------------------------------------------------+
int CStrategyMAinv::Initialization(int period,bool virtual_trade_flag)
  {
...
//+------------------------------------------------------------------+
int CStrategyMAinv::Deinitialization()
  {
...
//+------------------------------------------------------------------+
bool CStrategyMAinv::CheckTradeConditions()
  {
   ...
   // Buy condition: MA rises
   bool buy_condition_1=(m_values[0]>m_values[1]) && (m_values[1]>m_values[2]);
   // Buy condition 2: previous close price is above the moving average
   ...
   // Sell condition: // previous close price is lower than MA   
   bool sell_condition_2=(p_close<m_values[1]);

   int new_state=0;

   if(buy_condition_1  &&  buy_condition_2) new_state=SIGNAL_OPEN_SHORT;
   if(sell_condition_1 && sell_condition_2) new_state=SIGNAL_OPEN_LONG;

   if((GetSignalState()==SIGNAL_OPEN_LONG) && (buy_condition_1 || buy_condition_2)) new_state=SIGNAL_CLOSE_LONG;
   if((GetSignalState()==SIGNAL_OPEN_SHORT) && (sell_condition_1 || sell_condition_2)) new_state=SIGNAL_CLOSE_SHORT;
