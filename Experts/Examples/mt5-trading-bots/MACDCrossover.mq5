...
protected:
   ...
   bool              Crossover(SIGNAL trend_direction);
  };
//--- global expert
...
//+------------------------------------------------------------------+
bool CSampleExpert::Crossover(SIGNAL trend_direction)
  {
   bool res = false;
    if(trend_direction == BUY)
   //if(trend_direction == SELL)
     {
      // macd crosses above the signal line from bottom to top => bullish momentum => buy
   ...
   //   if(trend_direction == BUY)
        {
         // macd line crosses below the signal line from top to bottom => bearish momentum => sell
         if(m_macd_current < m_signal_current && m_macd_previous > m_signal_previous)
           {
            res = true;
           }
        ...
        {
         printf("Invalid trend direction: %i", trend_direction);
        }
   return(res);
  }
//+------------------------------------------------------------------+
